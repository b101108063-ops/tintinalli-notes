...
- **D-dimer**: Use an age-adjusted cutoff for patients >50 years (Age $\times$ 10 = upper limit). A normal result can rule out PE in low/intermediate probability scenarios, and rule out aortic dissection in low-risk patients.
- **CT Scan**:
...
- **Orthostatic**: Triggered by a change in position. Abnormal vitals: supine $\rightarrow$ standing $\downarrow\text{SBP} \ge \mathbf{20\text{ mmHg}}$ or $\downarrow\text{DBP} > \mathbf{10\text{ mmHg}}$.
...
- **Hypertension**: Goal $< 140/\mathbf{90\text{ mmHg}}$. Use ACEi (or ARB if intolerant).
